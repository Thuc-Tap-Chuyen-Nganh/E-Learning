...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("
        UPDATE lessons 
        SET title = ?, lesson_type = ?, video_url = ?, content = ?, duration = ?, sort_order = ?
        WHERE lesson_id = ?
    ");
    
    // "ssssii" -> string, string, string, string, int, int
    $stmt->bind_param("ssssiii", $title, $lesson_type, $video_url, $content, $duration, $sort_order, $lesson_id);

    if ($stmt->execute()) {
        header("Location: " . BASE_URL . "admin/admin_chapter_details.php?chapter_id=$chapter_id&status=updated");
    } else {
