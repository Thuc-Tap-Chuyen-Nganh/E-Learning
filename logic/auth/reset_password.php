...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($password) || empty($confirm)) {
        header("Location: " . BASE_URL . "reset_password.php?token=$token&error=empty");
        exit();
    }
    if ($password !== $confirm) {
        header("Location: " . BASE_URL . "reset_password.php?token=$token&error=mismatch");
        exit();
    }
    if (strlen($password) < 6) {
        header("Location: " . BASE_URL . "reset_password.php?token=$token&error=short");
        exit();
    }
    ...
    if (!$token_data || strtotime($token_data['expires_at']) < time()) {
        ...
        header("Location: " . BASE_URL . "forgot.php?error=invalidtoken");
        exit();
    }
    ...
    try {
        ...
        // (Này cũng tạm)
        header("Location: " . BASE_URL . "forgot.php?error=dberror");
        exit();
    }
